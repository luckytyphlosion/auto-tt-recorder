...
def main():
    ...
    ap.add_argument("-ttt", "--top-10-title", dest="top_10_title", default="auto", help="The title that shows at the top of the Top 10 Leaderboard. This option supports format specifiers which are listed at https://github.com/luckytyphlosion/auto-tt-recorder/blob/master/docs/DOCS.md. The default is {trackName} {cc} {vehicle} {category} {continent} Top 10. The format is smart in that if an option ends up being empty, it will consume the following space (i.e. so you don't get something like Mario Circuit  Glitch). Ignored if -ttg/--top-10-gecko-code-filename is specified.")
